// Live API smoke: one gold word must produce a multi-layer Reality Map.
// Prints no secrets. Prompt rewrite is out of this ticket.
...
if (!init.ok || initBody.phase !== "active" || !initBody.realityMap || layers < 2) {
  console.log("FAIL - live gold word did not produce a multi-layer tree");
  process.exit(1);
}
console.log("PASS - live gold word produced a multi-layer tree via OpenRouter");
