...
- [14 - Background function + client polling for generation](issues/14-background-function-polling.md) - the sync function's 30s cap killed generation at the 10-13 deploy (function logs: every failed request Duration 30000 ms while real generation takes 16-52s). The transport is now asynchronous end to end: the client sends a client-generated jobId in the POST body; the agent function is a Netlify BACKGROUND function (netlify.toml [functions.agent] background = true, 15-minute budget) that writes EVERY outcome as a terminal record to a Netlify Blobs store "agent-jobs" under job:<id> (30-min TTL) and never throws after the empty 202 (an uncaught throw re-runs the whole generation and double-spends tokens); a second SYNCHRONOUS function agent-status (GET /api/agent-status?job=<id>) reads the record and answers running/success/error (never platform HTML). callAgent posts, sees the 202 (background functions return an EMPTY 202 - no job id in the response, no built-in status endpoint), and polls the status endpoint every 2s (10-min client deadline) until the tree lands or a stable code collapses, with the exact same collapse rules as the old POST path; a non-202 POST (local dev, stubbed fetch) keeps the old sync behavior. Follow-up turns ride the same callAgent path - no fork. 353 tests + tsc clean; CDP 24/24 at 375/320 on the map entry flow (skeleton holds while the job runs, tree lands on poll success, no overflow, tap targets 44px). Screenshots in research/14-*.png; deploy + live verification post-14 by Hermes (2026-08-13).
