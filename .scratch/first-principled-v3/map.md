...
- [14 - Background function + client polling for generation](issues/14-background-function-polling.md) - the sync function's 30s cap killed generation at the 10-13 deploy (function logs: every failed request Duration 30000 ms while real generation takes 16-52s). The transport is now asynchronous end to end: the client sends a client-generated jobId in the POST body; the agent function is a Netlify BACKGROUND function (15-minute budget) that writes EVERY outcome as a terminal record to a Netlify Blobs store "agent-jobs" under job:<id> and never throws after the empty 202 (an uncaught throw re-runs the whole generation and double-spends tokens); a second SYNCHRONOUS function agent-status (GET /api/agent-status?job=<id>) reads the record and answers running/success/error (never platform HTML). callAgent posts, sees the 202 (background functions return an EMPTY 202 - no job id in the response, no built-in status endpoint), and polls the status endpoint every 2s (10-min client deadline) until the tree lands or a stable code collapses, with the exact same collapse rules as the old POST path; a non-202 POST (local dev, stubbed fetch) keeps the old sync behavior. Follow-up turns ride the same callAgent path - no fork. VERIFIER FIXES (Hermes, post-deploy): (1) background mode needs in-source `export const config = { background: true }` in agent.mjs - netlify.toml [functions.agent] alone is ignored by the direct CLI deploy (site is not git-integrated; first deploy still died at 30452 ms sync cap); (2) @netlify/blobs v10 set() does NOT serialize objects (stores "[object Object]"; the status endpoint 500'd on every read) - setJSON() is the correct call, used in both the job writer and the rate limiter (which had the same latent bug, silently failing open to in-memory). 353 tests + tsc clean; CDP 24/24 at 375/320 on the map entry flow (local + against the live URL). LIVE (deployed 6a7dd72e): microscope -> 11 nodes / 6 layers in ~48s via 202 + poll (old sync deploy died at 30s on the same concept); follow-up turn -> 14-node tree + reply. Screenshots in research/14-*.png (2026-08-13).
...
- Real per-layer progress: the skeleton lights on a fixed timer
  (SKELETON_STEP_MS) while the tree builds. Now that the client polls a
  background job, the status function could expose real per-layer progress
  (which layer bands actually landed) instead of the timer. Sharp enough to
  ticket once the background transport is live (14, 2026-08-13).
