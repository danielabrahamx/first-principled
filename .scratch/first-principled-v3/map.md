...
- [11 - Map word entry + shared generation + progressive skeleton](issues/11-map-word-entry-skeleton.md) - the map owns tree entry: a word input in the sticky header starts generation on the map (no route to chat), through the shared lib/generation.js module both pages use; a progressive skeleton mirrors the vertical-path tree and lights its bands bottom-up (foundation first) as each layer lands; chat becomes follow-up-only (start form removed). A refusal surfaces the model's reply on the page. Headless CDP 29/29 at 375/320 (input full-width, skeleton bottom-up reveal, tree lands on the map, no overflow). Screenshots in research/11-*.png for Danny; deploy deferred with 10-13 (2026-08-13).
