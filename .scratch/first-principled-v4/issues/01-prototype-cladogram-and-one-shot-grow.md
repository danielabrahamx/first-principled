...
- [ ] Prototype opens via a static server and shows a branching cladogram,
      not a single-column stack
- [ ] At time 0 of the grow, the root is visible; at end, every layer and
      card is visible without scrolling
- [ ] Scrolling the page after grow does not hide or fade layers
- [ ] Reduced-motion path shows the full Tree with no grow and no sap
- [ ] 375px: no accidental horizontal page overflow, or a documented
      stage-scroll if the v1 fit cannot hold
- [ ] Asset path linked from this ticket; Danny can react before ticket 03
