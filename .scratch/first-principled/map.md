...
- [18 - Security hardening](issues/18-security-hardening.md) - `resolved` (2026-08-08) - function wrapper now gates every request before any LLM call: 64KB body cap (413 too_large), per-IP hourly rate limit in a Netlify Blobs store (RATE_LIMIT_MAX, default 60, fail-open), Cloudflare Turnstile verification when TURNSTILE_SECRET_KEY is set (single-use token, action agent_turn, fail-closed); orchestrator caps word at 100 chars, message content at 10000 chars, history at last 60 messages; validator caps reality map at 60 nodes/120 edges/12 layers; client: invisible Turnstile widget per submit (src/turnstile.js, site key placeholder), token carried in the body, new stable error codes too_large/rate_limited/captcha_required/captcha_failed with friendly wording; src/_headers adds CSP (same-origin + Google Fonts + challenges.cloudflare.com), nosniff, DENY framing, referrer, permissions, COOP. 188 tests green, tsc clean, live-verified on prod (413/429/bad JSON/headers; normal turn works). Turnstile activates by setting the secret + site key.
