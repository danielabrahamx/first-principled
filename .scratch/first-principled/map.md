...
- [18 - Security hardening](issues/18-security-hardening.md) - `resolved` (2026-08-08) - function wrapper gates every request before any LLM call: 64KB body cap (413 too_large), per-IP hourly rate limit (RATE_LIMIT_MAX, default 60; Netlify Blobs store, in-memory fallback), Cloudflare Turnstile verification when TURNSTILE_SECRET_KEY is set (single-use token, action agent_turn, fail-closed); orchestrator caps word at 100 chars, message content at 10000 chars, history at last 60 messages; validator caps reality map at 60 nodes/120 edges/12 layers; client: invisible Turnstile widget per submit (src/turnstile.js, site key placeholder), token carried in the body, new stable error codes too_large/rate_limited/captcha_required/captcha_failed with friendly wording; src/_headers adds CSP (same-origin + Google Fonts + challenges.cloudflare.com), nosniff, DENY framing, referrer, permissions, COOP. 194 tests green (7 rate-limiter unit tests), tsc clean. Live-verified on prod: 413/400/word-cap/headers present/full init turn works. NOT live-verified: 429 - Netlify does not inject the blob context into this function on the free plan and free-tier cold starts defeat in-process counters, so the limiter is best-effort until blob context exists or the site sits behind a proxy. Turnstile activates by setting the secret + site key (the real wall; pending Danny's Cloudflare widget).
...
- [14 - Error banner always visible](issues/14-error-banner-always-visible.md) - ticket 13 CSS regression fixed with a global `[hidden] { display: none !important; }` guard in src/styles.css; CDP-verified hidden/visible/hidden cycle; 169 tests + tsc clean.
- [15 - Reality map viewable from session start](issues/15-reality-map-viewable-from-session-start.md) - Danny's product call (2026-08-08): the reality tree opens at session start, not session end. map.js gates the Reality tab/tree on realityMap !== null (comparison metrics stay session-end only); socratic rule 1 rewritten (learner may open the map, tutor never quotes it unprompted); spec 9 + AGENTS.md no-leak re-scoped to chat-only. CDP-verified mid-session; 169 tests + tsc clean.
- [16 - Briefing mode: tutor delivers information for decisions](issues/16-briefing-mode-tutor-delivers-information.md) - learner-initiated briefing (probe kind "brief"): briefingRequested(utterance) code-level matcher (explicit tell-me, "brief me on X", whole-path/end-to-end surveys, decision questions; single-concept what-is stays an explanation request); outranks the explanation gate and the opening rule; validateTurn demands kind brief when requested and rejects an unrequested brief (the model cannot lecture on its own); a briefing resets all failed-attempt counts; the ONE sanctioned mode for quoting the reality map - accurate, never embellished, no closing Socratic question; learner map still updates from the learner's post-briefing demonstrations (sim session: nodes flip correct, closures accumulate). Spec 8/9 + glossary + AGENTS.md no-leak exception. Live-verified on real DeepSeek (direct chain briefing, accurate quotes, gate cleared) and via the netlify dev HTTP flow; 51/51 socratic tests; the 1 unrelated failing test is ticket 17's in-flight tree work.
- [17 - Tree should read as phylogenetic lineage](issues/17-tree-should-read-as-phylogenetic-lineage.md) - treeLayout diverges branches from a central trunk at successive depths (new TREE_DIVERGENCE_STEP 36 per layer: top layer apps highest/nearest the crown, deepest foundation physics lowest; per-branch divergenceY replaces the single branchLineY); branch columns alternate left and right of the trunk (even right, odd left), deepest nearest the trunk per side, so elbows never cross cards (unit-tested invariant); odd branch counts leave the trunk the middle gap with a balanced box (live maps are often 5-layer); cladogramPaths = trunk root-bottom to deepest divergence + one elbow per branch + in-branch connectors; card metrics unchanged, geometry-only (AC5). npm test 184 green, tsc clean, netlify build OK; local CDP probe 12/12 (6 labels 156..336px strictly increasing, trunk M 920 74 V 326, 6 elbows, 8 cards, no overflow at 375px); deployed to prod + live CDP probe 7/7 on a real 5-layer map (trunk M 608 74 V 290). Spec 9 + research/13-ui-design-spec.md updated in the same commit.
...
16. [Briefing mode: tutor delivers information for decisions](issues/16-briefing-mode-tutor-delivers-information.md) - `resolved` (2026-08-08) - learner-initiated briefing (probe kind "brief"): briefingRequested matcher (explicit tell-me / brief me on X / whole-path & end-to-end surveys / decision questions, conservative so ordinary answers and single-concept asks stay Socratic), outranks the explanation gate, validateTurn enforces brief only-when-requested, briefing resets failed-attempt counts, the ONE mode where quoting reality is allowed (accurate, never embellished, no closing question), learner map still updates from post-briefing demonstrations; spec 8/9 + glossary + AGENTS.md updated; live-verified on real DeepSeek + netlify dev HTTP flow; 51/51 socratic tests (1 unrelated fail is ticket 17's in-flight tree work). Frontier continues at 17.
17. [Tree should read as phylogenetic lineage](issues/17-tree-should-read-as-phylogenetic-lineage.md) - `resolved` (2026-08-08) - tree.js reworked: branches now diverge from a central trunk at successive depths (top layer highest, deepest foundation lowest = chronological lineage), alternating left/right of the trunk so no elbow crosses another branch's cards; cladogramPaths draws trunk + per-divergence elbows; tree.test.js + spec 9 + design spec updated. 188 tests green, tsc clean, netlify build OK, deployed live. Both frontier tickets closed.
