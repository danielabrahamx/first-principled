...
test("two same-rank convergence extras never land on the same spot", () => {
  ...
  // foundation. Both extras would naively claim col -1 (the per-node slot
  // counter resets), stacking B and D on the same (x, y) - Danny's laptop
  // report. Columns are claimed per rank, so the second extra moves out.
  const map = {
    concept: "x",
    layers: [
